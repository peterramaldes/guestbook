...
The `home-routes` handler in the `swag-app.routes.home` namespace
...
The `home-page` function will in turn call the `swag-app.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `swag-app.handler` namespace:
...
Request middleware functions are located under the `swag-app.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `swag-app.dev-middleware` namespace found in
